...
LSZ2 = subset(ctbmn_grptyp, samp_ev == "LSZ2")
WLD2 = subset(ctbmn_grptyp, samp_ev == "WLD2")
YBP1 = subset(ctbmn_grptyp, samp_ev == "YBP1")
YBP2 = subset(ctbmn_grptyp, samp_ev == "YBP2")
SJR1 = subset(ctbmn_grptyp, samp_ev == "SJR1")
SJR2 = subset(ctbmn_grptyp, samp_ev == "SJR2")
```

####SJR1 stacked barchart with experiments
```{r}
library(ggplot2)
ggplot(data= SJR1, aes(x= reorder(grp_typ, tot_ct, FUN = sum), y=tot_ct, fill=exp))+
...
spp_mat_sh$Category <- spp_ts$Category

# 303 columns, 303 is Category

# make graph
library(plotly)
library(ggfortify)

spp_mat_sh[is.na(spp_mat_sh)] <- 0

# remove category
spp_mat_sh_g <- spp_mat_sh[,0:61]

spp_mat_sh_g <- log1p(spp_mat_sh_g)

pca_res <- prcomp(spp_mat_sh_g, scale.=F)

p <- autoplot(pca_res, data=spp_mat_sh, colour= 'Category', labels=T, shape = F, label.size=5)
ggplotly(p)
